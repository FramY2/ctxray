...
describe("parseCapabilityLock", () => {
  it("rejects malformed or unsupported lockfiles", () => {
    ...
    ).toThrow(/valid CtxWise capability lockfile/i);
  });

  it("still accepts lockfiles generated by the former ctxray identity", () => {
    const legacy = lock("2026-08-09T10:00:00.000Z", [
      {
        scope: "project",
        path: "AGENTS.md",
        sha256: "a".repeat(64),
    ...
    legacy.generator.name = "ctxray";
    expect(parseCapabilityLock(legacy).generator.name).toBe("ctxray");
  });
